...
def _evidence_workspace(
    ...
    """Resolve a run workspace and its descriptor-declared files safely."""
...
def _write_report_request(
    ...
    """Write the correlated PDF request and return its UTC generation timestamp."""
...
def _name_report_outputs(
    ...
    """Apply the shared host/timestamp stem to detached PDF and ZIP artifacts."""
...
def _report_argv(
    ...
    """Expand a descriptor renderer command without invoking a shell."""
    mapping = {
...
def _execute_report_argv(argv: list[str], workdir: Path, timeout_s: int) -> tuple[list[str], str]:
    """Execute a renderer command and return its combined output."""
    proc = subprocess.run(
        argv,
        cwd=str(workdir),
        capture_output=True,
...
def _run_report_renderer(
    ...
    """Render the PDF through the descriptor-declared executable without a shell."""
    argv = _report_argv(chain, paths, request_path, output_paths)
    return _execute_report_argv(argv, workdir, chain.get("timeout_s", 300))


def run_evidence_report(
    chain: dict[str, Any], artifact_path: str | None
) -> tuple[bool, dict[str, Any]]:
    """Compose the prior scan directory into a PDF-backed results bundle."""
    ...
    try:
        ...
        argv, log = _run_report_renderer(chain, workdir, paths, request_path, output_paths)
        missing = [
            str(path) for path in output_paths.values() if not path.is_file()
        ]
        if missing:
