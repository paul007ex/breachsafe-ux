    ...
    with gr.Blocks(title=BRAND["name"]) as demo:
        ...
        for did, desc in descs.items():
            ...
            with gr.Tab(desc.get("title", did)):
                ...
                for spec in desc.get("inputs", []):
                    ordered.append(
                        next(adv_iter) if spec.get("group") == "advanced" else next(basic_iter)
                    )
                ...
                for chain in desc.get("chains", []):
                    ...
                    if target is None or not tool_available(target):
                        ...
                        gr.Markdown(
                            f"_Requires `{need}`, which is not installed in this deployment._"
                        )
                        continue
                    ...
                    (
                        cbtn.click(
                            lambda: gr.update(value=_BUSY_LABEL, interactive=False), None, cbtn
                        )
                        .then(
                        ...
                        .then(
                            lambda c=chain: gr.update(
                                value=c.get("label", c["to"]), interactive=True
                            ),
                            None,
                            cbtn,
                        )
