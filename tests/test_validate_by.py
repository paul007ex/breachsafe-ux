...
def test_by_selects_matching_case():
    v = {"by": ["format"],
         "cases": {"cbom": {"argv": ["c"], "badge_rule": {}}, "json": None}}
...
def test_by_uses_default_when_no_case_matches():
    v = {"by": ["format"], "cases": {"cbom": {"argv": ["c"], "badge_rule": {}}},
...
def test_multi_key_joins_with_pipe():
    v = {"by": ["format", "profile"],
         "cases": {"cbom|strict": {"argv": ["cs"], "badge_rule": {}}}}
...
def test_validate_runs_selected_case(tmp_path):
    art = tmp_path / "a.json"
    art.write_text("{}")
    desc = {"validate": {"by": ["format"], "cases": {
        "cbom": {"argv": [sys.executable, "-c", "import sys; sys.exit(0)"],
...
def test_schema_rejects_validate_with_both_argv_and_by():
    d = {"schema_version": 1, "id": "t", "run": {"base": ["x"]},
         "validate": {"argv": ["a"], "badge_rule": {"pass_if": {"exit": 0}},
                      "by": ["format"], "cases": {}}}
