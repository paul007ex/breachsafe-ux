# breachsafe-wizard - known issues
...
- **W-1 descriptor dir not overridable / DESCS bound at import.** `load_descriptors()` globs a
  hardcoded `TOOLS = ROOT/"tools"` (`src/breachsafe_wizard/facade.py:14,22`) and `app.py:20` binds
  `DESCS = load_descriptors()` at import. A host package cannot point the wizard at its own
  descriptor. Fix: read `WIZARD_TOOLS_DIR` at call time; load DESCS lazily in `build()`.
- **W-2 `main()` ignores host.** Hardcodes `server_name="127.0.0.1"` (`app.py:224`); `WIZARD_HOST`
  is unread. Fix: read `WIZARD_HOST`.
...
- **W-5 (host-side) import order.** Any host launching the wizard must set `WIZARD_TOOLS_DIR` before
  importing `breachsafe_wizard.app` (depends on W-1). Document it.
