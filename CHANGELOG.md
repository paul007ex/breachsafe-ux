...
- Make Test Connection select the shared Raw Log tab after it runs, for immediate TLS/SSH
  handshake feedback.
...
[![Version](https://img.shields.io/badge/version-0.13.8-blue?style=flat-square)](CHANGELOG.md)
...
### Fixed
- Remove unused binary override environment variables from the customer image; the evidence
  descriptor is the single source of truth for its CLI invocation.

## [0.13.5] - 2026-08-24

### Fixed
- Preserve the full UUID entropy in per-run artifact directory names.

## [0.13.4] - 2026-08-24

### Fixed
- Keep the container health check below the enforced complexity ceiling while reporting every
  configured tool dependency.

## [0.13.3] - 2026-08-24

### Fixed
- Fail the customer image build when the evidence binaries or PDF renderer checksum are
  missing or invalid.
- Run the EnXemble container as a dedicated non-root user and reject PDF links outside the
  current runtime artifact root.

## [0.13.2] - 2026-08-25

### Changed
- Updated the security policy, Docker ADR, and architecture diagrams for the current EnXemble
  product name, `0.13.x` support line, and actual Gradio boundary.
- Replaced the unreliable external Scorecard badge endpoint with the repository workflow badge.

## [0.13.1] - 2026-08-25

### Fixed
- Align the container build label and runtime `BREACHSAFE_UX_BUILD_VERSION` with the packaged
  application version.
...
### Fixed
- Restored green CI, release, CodeQL, and OpenSSF gates after the EnXemble evidence-surface
  rework.
- Reduced UI orchestration complexity without changing the descriptor-driven execution contract.
- Ensured optional PDF export honors its feature flag and does not run when its tool is absent.
...
### Added
- **Evidence PDF export.** The Go evidence/PDF toolchain is wired into the host, so a scan can
  be exported as a portable evidence pack without leaving the UI. New `src/breachsafe_ux/
  evidence.py` and `evidence_ui.py`, declared through the descriptor schema so the wiring stays
  config-driven and tool-agnostic. Covered by `tests/test_evidence.py` and an end-to-end test.

### Changed
- Every stale `Python 3.12` reference now reads `3.14`, matching `requires-python` which has
  required `>=3.14` for several releases. Nine files, including the README badge a visitor sees
  first. Migration #100 is complete; the notes describing it as pending are removed.
